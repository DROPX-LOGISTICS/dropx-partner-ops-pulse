...
export default async function AssociateCapacityPage(
  ...
  return <AppShell active="Capacity" pageCode="cps_associates"><div className="ops-command-center capacity-workspace">
    ...
    <section className="panel capacity-associate-delivery-detail">
      <div className="panel-head"><div><h2>Pincode & package mix</h2><p className="subtle">Delivered Detail is used only for pincode and package-size analysis.</p></div><span className="status-pill neutral">MTD · {monthStart.split("-").reverse().join("/")}–{end.split("-").reverse().join("/")}</span></div>
      ...
      <div className="performance-summary-grid">
        ...
        <article><span>MTD detail rows</span><strong>{fmt(mtdDelivered)}</strong><small>{pincodes.length} pincodes served</small></article>
