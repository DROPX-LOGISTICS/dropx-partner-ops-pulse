...
function parseWorkerRefreshProgress(raw: unknown): Record<string, unknown> | null {
...
async function peekNextCiaStation(runId?: string): Promise<{
  ...
  run: CiaNetworkRefreshResult["run"];
  ...
  return {
    ...
    window: windowRaw?.from && windowRaw?.to
      ...
      : null,
    refreshProgress: parseWorkerRefreshProgress(raw.refreshProgress)
  };
}
 ...
 * refresh path (same as the row Refresh button). Do not call the worker's
 * nested self-fetch continue tick — that path often queues retries.
 */
