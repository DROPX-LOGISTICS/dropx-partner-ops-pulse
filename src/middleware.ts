...
import { isLocalDevHost, isOpsRequestHost } from "@/lib/ops-host";
...
export async function middleware(request: NextRequest) {
  ...
  if (
    ...
    // Local Ops-only: keep user on this app instead of sending them to production dashboard.
    if (isLocal) {
      return NextResponse.redirect(new URL("/" + request.nextUrl.search, request.url));
    }
